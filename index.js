...
async function handleApiRequst(urlMethod, requestMethod, data = {}) {
  ...
  if (instanceData.apiTokenInstance && instanceData.idInstance) {
    try {
      const url = `${config.baseUrl}/waInstance${instanceData.idInstance}/${urlMethod}/${instanceData.apiTokenInstance}`;
      const response = await fetch(url, {
        method: requestMethod,
        body:
          requestMethod === "POST" ? JSON.stringify(data, null, 4) : undefined,
      });
...
async function sendMessage() {
  ...
  try {
    const messageData = {
      chatId: messageChatIdInput.value,
